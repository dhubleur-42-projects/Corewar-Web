...
kubectl create configmap exec-app-config --from-env-file=/tmp/exec-app.env -n corewar-prod --insecure-skip-tls-verify --dry-run=client -o yaml | kubectl apply --insecure-skip-tls-verify -f -
...
kubectl delete job db-migration -n corewar-prod --insecure-skip-tls-verify --ignore-not-found
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  if [[ "$manifest" == "01-migration.yaml" ]]; then
    kubectl wait -n corewar-prod --for=condition=complete job/db-migration --timeout=300s --insecure-skip-tls-verify
