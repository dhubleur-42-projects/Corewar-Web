...
kubectl create namespace corewar-mr-${MR_ID} --dry-run=client -o yaml | kubectl apply -f -
...
if kubectl get secret data-credentials -n corewar-mr-${MR_ID} >/dev/null 2>&1; then
  POSTGRES_PASSWORD=$(kubectl get secret data-credentials -n corewar-mr-${MR_ID} -o jsonpath="{.data.postgres-password}" | base64 --decode)
  REDIS_PASSWORD=$(kubectl get secret data-credentials -n corewar-mr-${MR_ID} -o jsonpath="{.data.redis-password}" | base64 --decode)
  JWT_SECRET=$(kubectl get secret data-credentials -n corewar-mr-${MR_ID} -o jsonpath="{.data.jwt-secret}" | base64 --decode)
else 
...
kubectl create configmap back-app-config --from-env-file=/tmp/back-app.env -n corewar-mr-${MR_ID} --dry-run=client -o yaml | kubectl apply -f -
...
kubectl create configmap exec-app-config --from-env-file=/tmp/exec-app.env -n corewar-mr-${MR_ID} --dry-run=client -o yaml | kubectl apply -f -
...
kubectl delete job db-migration -n corewar-mr-${MR_ID} --ignore-not-found
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  kubectl apply -f /tmp/${manifest} -n corewar-mr-${MR_ID} --validate=false

  if [[ "$manifest" == "02-migration.yaml" ]]; then
    kubectl wait -n corewar-mr-${MR_ID} --for=condition=complete job/db-migration --timeout=300s
    POD=$(kubectl get pod -n corewar-mr-${MR_ID} -l job-name=db-migration -o jsonpath='{.items[0].metadata.name}')
    echo ====== Migration job logs ======
    kubectl logs $POD -n corewar-mr-${MR_ID}
    echo ================================
  fi

  if [[ -n "${DEPLOYMENTS[$manifest]}" ]]; then
    for deploy in ${DEPLOYMENTS[$manifest]//,/ }; do
      kubectl rollout status deployment/$deploy -n corewar-mr-${MR_ID} --timeout=180s
    done
  fi
done
